<tr>
    ...
    @endphp
    <div class="flex items-center space-x-2 bg-gray-100 text-gray-700 px-3 py-1.5 rounded-full shadow-sm hover:bg-gray-200 transition">
        <!-- Icono decorativo -->
        ...
        <div class="flex space-x-1">
            <!-- Botón editar -->
            <button wire:click="$set('editingAttributeId', {{ $attribute->id }})" class="text-indigo-500 hover:text-indigo-700 transition-colors duration-200">
            ...
            <!-- Botón borrar -->
            @if ($attribute)
                <button wire:click="removeAttribute({{ $attribute->id }})" class="text-red-500 hover:text-red-700 transition-colors duration-200">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    @if ($editingAttributeId == $attribute->id)
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">Editar Atributo</h2>

                <!-- Formulario de edición -->
                <form wire:submit.prevent="editAttribute({{ $attribute->id }}, $refs.editAttributeValue.value)">
                    <div class="mb-4">
                        <label class="block text-gray-700 font-medium mb-1">Valor</label>
                        <input wire:model="attributeValue" type="text" x-ref="editAttributeValue"
                            class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400"
                            value="{{ $attributeValue }}">
                    </div>

                    <div class="flex justify-end space-x-2">
                        <button type="button" wire:click="$set('editingAttributeId', null)"
