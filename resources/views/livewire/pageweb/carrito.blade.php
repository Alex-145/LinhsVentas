<div x-data="{ isOpen: @entangle('isOpen') }">
    ...
    <div x-show="isOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50" @click.self="isOpen = false">
        <!-- Modal Content -->
        <div x-show="isOpen" x-transition:enter="transition transform duration-300 ease-in-out"
            x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
            x-transition:leave="transition transform duration-300 ease-in-out" x-transition:leave-start="translate-x-0"
            x-transition:leave-end="translate-x-full" class="fixed inset-0 flex justify-end">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <img src="{{ asset('storage/' . $item['photo_url']) }}"
                                            alt="{{ $item['name'] }}" class="w-16 h-16 object-cover mr-4">
                                        <div>
